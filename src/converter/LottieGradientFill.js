...
class LottieGradientFill extends LottieShapeContent {
    ...
    findInterpolatedValue(prevKey, nextKey, time) {
        ...
        let interpolationFunction = bezierEaser.getBezierEasing(
            prevKey.o.x,
            prevKey.o.y,
            prevKey.i.x,
            prevKey.i.y
        ...
        if (Array.isArray(prevKey.s)) {
            return prevKey.s.map((value, index) =>{
                return value + (nextKey.s[index] - value) * perc;
            })
        } else if( typeof(prevKey.s) === 'object') {
            const newObj = {};
            for (var s in prevKey.s) {
    ...
    serialize() {
        ...
        const properties = serializedStops.map(stop => {
            return [stop.p, stop.c];
        }).flat();
    ...
    _startX = new LottieNumberProperty(0);
    _startY = new LottieNumberProperty(0);
    _endX = new LottieNumberProperty(0);
    _endY = new LottieNumberProperty(0);
    _opacity = new LottieNumberProperty(0);
