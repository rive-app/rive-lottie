...
const findInterpolatingProperty = (ranges, keyframeNumbers, currentRangeIndex, propertyIndex) => {
    ...
    let lowerKeyframeValue, upperKeyframeValue
    while (lowerIndex >= 0) {
        const keyframe = keyframeNumbers[ranges[lowerIndex]];
        let prop = keyframe.find(prop => prop.propertyIndex === propertyIndex)
        if (prop) {
...
const findMissingIndexes = (initKeyframeData, endKeyframeData, totalProperties) => {
    return Array
        .from(Array(totalProperties),(x,i)=>i)
        .filter(index => {
            return !(initKeyframeData.find(keyframeData => keyframeData.propertyIndex === index)
...
const areInterpolationsEqual = (properties) => {
    let lastI = JSON.stringify(properties[0].keyframe.i);
    let lastO = JSON.stringify(properties[0].keyframe.o);
